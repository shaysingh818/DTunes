...
#[tauri::command]
pub fn delete_genre(user_db_path: &str, genre_id: &str, user_thumbnail_path: &str) -> String {
    let conn = match Connection::open(user_db_path) {
        Ok(connection) => connection,
        Err(e) => {
            println!("{:?}", e);
...
#[tauri::command]
pub fn remove_audio_file_genre(
    user_db_path: &str,
    genre_id: &str,
    audio_file_id: usize,
) -> Result<String, String> {

...
export async function downloadSQLiteFile() {
  try {
    ...
    const fullPath = joinPaths('dtunes-audio-app', 'metadata', 'dtunes-audio-app.sqlite3');
...
export async function downloadDefaultAlarmSound(fileName: string) {
    ...
    const arrayBuffer = await fileBlob.arrayBuffer();
    const uint8Array = new Uint8Array(arrayBuffer);
    const fullPath = joinPaths('dtunes-audio-app', 'audio_files', fileName);
...
export async function downloadDefaultThumbnail(fileName: string) {
    ...
    const arrayBuffer = await fileBlob.arrayBuffer();
    const uint8Array = new Uint8Array(arrayBuffer);
    const fullPath = joinPaths('dtunes-audio-app', 'images', fileName);
...
export async function createAppDataFolders() {
  ...
  try {
    dirs.forEach((dir) => {
        const fullPath = joinPaths(appName, dir);
...
export async function syncMetadata() {
  if(await checkAppDataFolders() == false) {
    ...
    console.log("Donwloaded pomodoro alarm sound to audio files directory...")  
